...
from cointegration_tests import (
    matrix_ols_regression, adf_results, kpss_results,
    engle_granger, engle_granger_bidirectional, za_test,
    analyze_error_correction_model, ou_params, select_var_order, johansen,
    analyze_regression_var_summary, analyze_ecm_timeslices,
    analyze_johansen_triples, johansen_sensitivity_summary
)
...
class TestJohansenCointegration:
    ...
    def test_johansen_triples_analysis(self, make_stable_VAR2):
        ...
        var_data = make_stable_VAR2(T=500)
        var_data['var3'] = 0.5 * var_data['var1'] + 0.3 * var_data['var2'] + 0.1 * np.random.randn(len(var_data))
        ...
        if not result.empty:
            expected_cols = {'triple', 'n_assets', 'data_points', 'n_coint_relations', 
                           'first_eigenvec_norm', 'spread_vol'}
            assert set(result.columns) == expected_cols
