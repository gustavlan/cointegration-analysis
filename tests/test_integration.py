...
class TestPairsTradeIntegration:
    ...
    def test_small_portfolio_end_to_end(self, sample_portfolio_data):
        ...
        cointegrated_pairs = [
            pair for pair, result in cointegration_results.items()
        ...
        with patch('plotting.plt.show'):
            summary_df, opt_tables = analyze_pairs_nb(
                portfolio['all_data'], selected_subset,
                Z_min=1.5, Z_max=2.5, dZ=0.5  # Coarse grid for speed
            )
        ...
        cv_results, cv_artifacts = run_cv_over_pairs(
            portfolio['all_data'], selected_subset, z_threshold_by_pair,
            n_splits=2, return_artifacts=True
        )
    ...
    def test_rolling_vs_static_comparison(self, make_cointegrated_pair):
        ...
        rolling_result = backtest_with_rolling_cointegration(
            data['y'], data['x'], z_threshold=2.0, 
            window_size=150, step_size=30, train_ratio=0.4
        )
        ...
        for result, name in [(static_result, 'static'), (rolling_result, 'rolling'), (kalman_result, 'kalman')]:
...
pytest.mark.slow = pytest.mark.skipif(
    not pytest.config.getoption("--slow", default=False),
    reason="Slow test - run with --slow flag"
)
