...
from backtests import (
    calculate_performance_metrics, align_price_data, split_train_test,
    estimate_cointegration, generate_trading_signals, calculate_strategy_returns,
    compute_drawdowns, compute_rolling_sharpe, compute_rolling_beta,
    backtest_pair_strategy, compute_ts_folds, run_cross_validation_backtest,
    run_cv_over_pairs, summarize_cv, stitch_cv_folds, run_systematic_backtest,
    backtest_with_rolling_cointegration, backtest_with_kalman_filter,
    rolling_cointegration_analysis, adaptive_cointegration_analysis,
...
class TestPerformanceMetrics:
    ...
    def test_calculate_performance_metrics_basic(self, numerical_tolerances):
        ...
        expected_keys = {'total_return', 'annualized_return', 'annualized_volatility', 'sharpe_ratio'}
    ...
    def test_calculate_performance_metrics_empty_series(self):
        ...
        expected = {'total_return': 0, 'annualized_return': 0, 'annualized_volatility': 0, 'sharpe_ratio': 0}
...
class TestStrategyReturns:
    ...
    def test_calculate_strategy_returns_basic(self):
        ...
        expected_keys = {'strategy_returns', 'spread_returns', 'asset1_returns', 
                        'asset2_returns', 'cumulative_returns', 'positions_used'}
        assert set(result.keys()) == expected_keys
        
        # Check that positions are lagged (avoid look-ahead bias)
        expected_lagged_pos = pd.Series([np.nan, 1, 0, -1, 0], index=dates)
        pd.testing.assert_series_equal(result['positions_used'], expected_lagged_pos, check_dtype=False)
    ...
    def test_calculate_strategy_returns_beta_neutral(self):
        ...
        pd.testing.assert_series_equal(result['spread_returns'], expected_spread_rets, check_exact=False)
...
class TestCrossValidation:
    ...
    def test_run_cross_validation_backtest(self, make_cointegrated_pair):
        ...
        expected_cols = {
            'split', 'z_threshold', 'hedge_ratio', 'total_return', 'sharpe_ratio',
            'max_drawdown', 'num_trades', 'train_start', 'train_end', 'test_start', 'test_end'
        }
    ...
    def test_summarize_cv(self, sample_portfolio_data):
        ...
        expected_cols_pattern = ['total_return_mean', 'total_return_std', 'sharpe_ratio_mean', 
                               'sharpe_ratio_std', 'max_drawdown_mean', 'num_trades_mean']
    ...
    def test_stitch_cv_folds(self, sample_portfolio_data):
        ...
        mock_artifacts = {
            pair_name: {
                (0, z_threshold): {
                    ...
                    'cumulative_returns': pd.Series([1.01, 1.0302, 1.020398],
                                                  index=pd.date_range('2020-01-01', periods=3, freq='B')),
                    'positions': pd.Series([1, 1, -1],
                                         index=pd.date_range('2020-01-01', periods=3, freq='B')),
                    'spread': pd.Series([0.1, 0.2, -0.1],
                                      index=pd.date_range('2020-01-01', periods=3, freq='B'))
                },
                (1, z_threshold): {
                    'strategy_returns': pd.Series([0.005, -0.01], 
                                                index=pd.date_range('2020-01-06', periods=2, freq='B')),
                    'cumulative_returns': pd.Series([1.005, 0.9950],
                                                  index=pd.date_range('2020-01-06', periods=2, freq='B')),
                    'positions': pd.Series([0, -1],
                                         index=pd.date_range('2020-01-06', periods=2, freq='B')),
                    'spread': pd.Series([0.05, -0.15],
                                      index=pd.date_range('2020-01-06', periods=2, freq='B'))
                }
            }
        }
        ...
        expected_keys = {'strategy_returns', 'cumulative_returns', 'positions', 
                        'spread', 'drawdowns', 'fold_boundaries', 'performance_metrics'}
...
class TestFullBacktestIntegration:
    ...
    def test_backtest_pair_strategy_integration(self, make_cointegrated_pair):
        ...
        expected_keys = {
            'split_info', 'cointegration', 'signals', 'returns', 
            'drawdowns', 'test_spread', 'performance_metrics'
        }
        assert set(result.keys()) == expected_keys
        ...
        expected_perf_keys = {
            ...
            'sharpe_ratio', 'max_drawdown', 'num_trades', 'avg_return_per_trade', 'win_rate'
        }
    ...
    def test_backtest_with_no_signals(self, make_cointegrated_pair):
        ...
        result = backtest_pair_strategy(
            data['y'], data['x'],
            z_threshold=5.0,  # Very wide threshold
            train_ratio=0.6
        )
...
class TestRollingAnalysis:
    ...
    def test_backtest_with_rolling_cointegration(self, make_cointegrated_pair):
        ...
        expected_keys = {
            'strategy_returns', 'cumulative_returns', 'beta_history', 
            'drawdowns', 'performance_metrics', 'parameters'
        }
        assert set(result.keys()) == expected_keys
    ...
    def test_backtest_with_kalman_filter(self, make_cointegrated_pair):
        ...
        result = backtest_with_kalman_filter(
            data['y'], data['x'],
            z_threshold=2.0,
            train_ratio=0.5
        )
        ...
        expected_keys = {
            'strategy_returns', 'cumulative_returns', 'adaptive_betas', 'performance_metrics', 'parameters'
        }
        assert set(result.keys()) == expected_keys
    ...
    def test_adaptive_cointegration_analysis(self, sample_portfolio_data):
        ...
        if not summary_df.empty:
            expected_summary_cols = {
                'Pair', 'Static_Beta', 'Initial_Beta', 'Final_Beta',
                'Beta_Drift', 'Beta_Vol', 'Static_Sharpe', 'Kalman_Sharpe'
            }
