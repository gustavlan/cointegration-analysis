...
def backtest_with_rolling_cointegration(price1, price2, z_threshold=2.0, 
    ...
    while current_idx + step_size <= len(data):
        ...
        # Generate spread and signals for this period
        period_spread = (period_data['asset1'] - coint_result['alpha'] - 
                        coint_result['beta'] * period_data['asset2'])
        
        # Use historical spread statistics for z-score
        if len(all_returns) > 0:  # Use accumulating history
            hist_spread = pd.concat([pd.Series(dtype=float)] + 
                                   [pd.Series([s]) for s in all_returns])
            mean_spread = hist_spread.mean()
            std_spread = hist_spread.std()
        else:  # Use estimation period
        ...
        # Use previous period position (avoid look-ahead)
        if len(all_positions) > 0:
            prev_position = all_positions[-1] if len(all_positions) > 0 else 0
        else:
            prev_position = 0
            
        strategy_returns = prev_position * spread_returns
        
        # Store results
        all_returns.extend(strategy_returns.fillna(0).tolist())
        all_positions.extend(positions.tolist())
        
        current_idx += step_size
    
    # Convert to series
    strategy_returns = pd.Series(all_returns, 
                                index=data.index[test_start_idx:test_start_idx+len(all_returns)])
    positions = pd.Series(all_positions[:len(all_returns)], 
