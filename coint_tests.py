    ...
    for p in range(1, maxlags+1):
        try:
            ...
            records.append({
                ...
                'stable': all(abs(r) < 1 for r in res.roots), 'eigenvalues': res.roots
            })
        except Exception:
            continue
...
def johansen(df, freq="B", det_order=0):
    ...
    try:
        ...
        k_ar_diff = max(int(np.median([best_aic, best_bic, best_hqic])) - 1, 1)
    except Exception:
        k_ar_diff = 1
    res = coint_johansen(df.values, det_order=det_order, k_ar_diff=k_ar_diff)
    out = {'johansen_n': int(np.sum(res.lr1 > res.cvt[:, 1]))}
    if res.evec is not None and res.evec.shape[1] > 0:
        for i, w in enumerate(res.evec[:, 0]):
...
def za_test(series, trim=0.1, lags=None, model='trend'):
    s = series.dropna()
    regression = 'ct' if model == 'trend' else 'c'
...
def analyze_regression_var_summary(all_data):
    ...
    for name, df in all_data.items():
        ...
        r2 = 1 - ((y.values - X.values @ beta)**2).sum() / ((y.values - y.mean())**2).sum()
...
def analyze_ecm_timeslices(y, x, spread=None, periods=5, include_dX=True, reestimate_beta_per_slice=True, 
    ...
    for idx in slices:
        ...
        beta, alpha = model.params['x'], model.params['const']
        spread_local = sub['y'] - alpha - beta * sub['x']
        
        # ECM regression
        d_y = sub['y'].diff().dropna()
        u_lag = spread_local.shift(1).dropna()
